...
- published: 2026-07-28T20:54:18Z
- bundle size: 18.9 MB · 65 media files
- replica fidelity score: 0.8733 (from the report beside the published page)

`index.html` is the generated site itself. Everything about how it was made — every
artifact below, and the run-status disclosure — is on `pipeline.html`, one link away in
the site's footer.
...
- Replica fidelity is 0.8733 against this run's 0.90 bar — below the bar. (`replica/replica-report.json`)
- The gate did flag the weakest sections numerically: (unauthored) 0.0, featuresThreeColumnCard 0.8555, hero 0.8622. Low band scores are where the composed page diverges most from the source (1 band with no measurable source height is left out of this list). (`replica/replica-report.md`)
- Do not trust the run manifest over the report beside the page: the run manifest records status "completed" / pipeline_run_completed true, which the gate evidence above does not support; and the manifest's replica score (0.8206) predates the replica that is published here (0.8733 in the report beside the page). (`logs/manifest.json`)
...
- **Generated site (React + Vite)** — `index.html`  
...
- **Pipeline details** — `pipeline.html`  
  Run status, every rendered lane, the brand facts and the logs, in one page.
- **Brand facts** — `brand/`  
