#!/usr/bin/env python3
"""HubSpot fix-batch LIVE seeded rerun (2026-07-03).

Same live path + arguments as experiments/hubspot-validation/run_validation.py
(generate_composition.py: structured Anthropic call -> schema validate -> neverDo +
off-grid prefilters -> compose_from_composition render -> onbrand_check --composition
gate -> <=2 repairs), rerun AFTER the fix batch landed. Output goes to a NEW additive
dir runs/hubspot/brand/compose/signup-launch-fixed-live/ (the prior worker's dirs stay
read-only). The deterministic replay of the SAME red composition lives beside it in
signup-launch-fixed/ — that one isolates the fixes; this one proves the end-to-end
seeded path converges green on a fresh composition.

Usage:  ../../venv/bin/python run_live_fixed.py   (from this dir)
...
OUT_DIR = REPO / "runs" / "hubspot" / "brand" / "compose" / "signup-launch-fixed-live"
...
def main() -> int:
    ...
    t0 = time.time()
    # Same structural directive as run_validation.py (verbatim) so the A/B against the
    # 2026-07-03 tokenized run isolates the fix batch.
    directive = (
