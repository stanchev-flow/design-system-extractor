...
| [`font-availability-schema.md`](font-availability-schema.md) | The **typography availability** fact (`font-availability.v1`): the three delivery outcomes a declared family can have (`self-hosted` / `proxy-substituted` / `unavailable`), what `tools/extract/harvest_font_faces.py` writes from captured `@font-face` rules, the `selfHostedFonts:` + `fontAvailability:` brand.yaml keys, the licensing evidence limits (discovery is not permission), where a substitution is inserted in a stack and why, and the validator row still owed. |
