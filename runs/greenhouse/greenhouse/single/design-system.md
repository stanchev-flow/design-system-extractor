...
metadata:
  ...
  description: A calm, premium editorial aesthetic pairing high-contrast serif headings with a neutral humanist sans on deep-green
    inverse surfaces, mint and white canvases, and warm human-centered photography.
  confidence: high
tokens:
  color:
    surface:
  ...
  motion:
    note: Restrained, system-native transitions; subtle hover color/opacity shifts on links and buttons; optional gentle reveal
      on photography and cards.
surfaces:
  page_canvas:
    value: '#FFFFFF'
    role: page_canvas
  ...
  mint_canvas:
    ...
    gradient: none
    usage:
    - opening bookend / introductory banner canvas with floating media
    - soft full-width breathing band with edge-bleeding photography
    confidence: medium
  inverse_strip:
    value: '#15372C'
    role: inverse_run
    ...
    gradient: none
    usage:
    - thin promo/announcement strips
    - standalone notice bands with inline link
    confidence: high
  inverse_action_run:
    value: '#163328'
    role: inverse_run
    ...
    usage:
    - closing bookend call-to-action band with decorative brand motif
    confidence: high
  inverse_footer:
    value: '#14352B'
    role: inverse_run
    ...
    gradient: none
    usage:
    - site-wide closing footer with link columns, newsletter, legal
    confidence: high
  card_surface:
    value: '#FFFFFF'
    role: card
    ...
    gradient: none
    usage:
    - elevated white mini-cards over mint or light canvases
...
components:
  inline_text_link:
    ...
    variants:
      ...
      on_light:
        ...
        contrastSource: none
    doNotUseFor:
    - primary CTAs that need a filled pill
  primary_button:
    kind: button
    actualPageUI: true
    confidence: high
  ...
  feature_card:
    ...
    anatomy:
    - rounded media well
    - optional overlay label chip
    - serif heading
    - body paragraph
    - outlined CTA or text link
    base:
    ...
    doNotUseFor:
    - surfaces requiring a filled card chrome or border
  media_overlay_chip:
    kind: compact_label
    actualPageUI: true
    confidence: medium
    anatomy:
    - leading single-color glyph
    - label text
    - pill container
    base:
  ...
  divider_vertical:
    ...
    doNotUseFor:
    - high-contrast separation where whitespace suffices
  floating_widget:
    kind: panel
    actualPageUI: true
    confidence: medium
    ...
    doNotUseFor:
    - primary in-flow content panels
patterns:
  layout:
  - Constrained centered content column inside full-bleed section runs; section runs alternate light, mint, and deep-green
    inverse surfaces.
  - Three-up equal-width feature column grid on a light canvas, each column an image-led chromeless stack.
  - 'Split layout: left-aligned heading + single primary CTA column beside a multi-cell logo/proof grid, separated by a hairline
    vertical rule.'
  ...
  page_moments:
  - 'Opening bookend: introductory banner on a soft mint canvas with central messaging flanked by floating media and embedded
    product-UI snippets.'
  - 'Closing bookend: full-width deep-green call-to-action band with an oversized heading, single primary action, and a subtle
    decorative brand motif.'
  - 'Utility band: thin deep-green inverse strip carrying a single line of promo/notice text with a trailing bold underlined
    inline link.'
  - 'Inset child-panel behavior: white elevated cards float over mint or light canvases via radius + soft shadow.'
  content_composition:
  - 'Image-led cards: rounded media well on top, serif heading, short sans body, then an outlined pill CTA or underlined text
    link.'
  ...
  - Parent canvas color comes from edge/gutter continuity; cards and media wells remain child/inset surfaces unless they visibly
    become a full-width reset.
  - Deep-green inverse runs are localized bands, not the global page background; white is the default page canvas.
  - Adjacent deep-green runs may share a family hue while remaining distinct surfaces; do not merge a notice strip, CTA band,
    and footer into one surface.
  - Photography may bleed to section edges as foreground media rather than sitting inside a framed well when a band acts as
    breathing space.
  background_systems:
...
imagery:
  icons:
    ...
    creativeDirection: Minimal single-tone utility glyphs (search, dismiss, social, small leading marks) matching the local
      surface color.
    density: sparse
    simplicity: minimal
    rendering: line
    paletteRelationship: "single tone keyed to surface \u2014 dark glyphs on light, pale/light glyphs on inverse, knockout\
      \ glyphs inside accent chips"
    ...
    - heavy detail
    - treating logos as generated icons
  illustrations:
    observed: true
    creativeDirection: Sparse, subtle decorative brand motif (e.g. soft organic/leaf-like vector) blended into deep-green
      CTA backgrounds; barely-there tonal contrast.
    density: sparse
    simplicity: minimal
    ...
    subjectPolicy: style from design system; subject from local component slot
    avoid:
    - high-detail or multi-color illustration
    - literal scenes
    - foreground prominence
  interfaces:
    observed: true
    creativeDirection: "Clean product-UI snippets \u2014 white mini cards with avatar rows, dropdowns, bar charts, status\
      \ pills, and a dark-green candidate panel with overlapping profile thumbnails."
    density: moderate
    simplicity: simple
    ...
    subjectPolicy: style from design system; subject from local component slot
    avoid:
    - promoting embedded UI into real page UI
    - device mockups
  photography:
    observed: true
    creativeDirection: Warm, candid, human-centered workplace photography; soft natural lighting, realistic, slightly muted
      warm tones.
    density: moderate
    simplicity: moderate
    ...
    subjectPolicy: style from design system; subject from local component slot
    avoid:
    - studio-sterile or stocky compositions
    - baking rounded frames into the photo asset
rules:
  color:
  ...
  - Primary green pill (#4CB398) is used on light surfaces; blue (#2F6FE0) primary appears only on the deep-green CTA run;
    deep-green fill primary appears on the mint canvas.
  typography:
  ...
  cards:
  - Feature cards are chromeless on light canvases (no fill, no border, no shadow); separation is whitespace and the media
    well.
  ...
  - "Single-color icons are inline SVGs; logos/wordmarks and customer marks are never generated assets \u2014 fall back to\
    \ simple text/rectangle in the current surface color."
  - Photography is generated edge-to-edge; framing/rounding lives in the component.
  motion_animation:
  - 'Use restrained system-native transitions: hover color/opacity on links and buttons, gentle fade/translate reveals on
...
- Embedded product-UI cards, candidate panels, status badges, and floating mini-cards are showcase-only and must not become
  real reusable page UI.
...
- Customer logo marks render in native brand colors directly on white with no container; do not stylize, recolor, or frame
  them.
...
source_font_implementation:
  ...
  css: "@font-face {\n  font-family: \"Inter\";\n  src: url(\"source-fonts/01-inter-300-normal.ttf\") format(\"truetype\"\
    ...
    \  font-display: swap;\n}\n\n@font-face {\n  font-family: \"Inter\";\n  src: url(\"source-fonts/03-inter-500-normal.ttf\"\
    ) format(\"truetype\");\n  font-weight: 500;\n  font-style: normal;\n  font-display: swap;\n}\n\n@font-face {\n  font-family:\
    \ \"Inter\";\n  src: url(\"source-fonts/04-inter-600-normal.ttf\") format(\"truetype\");\n  font-weight: 600;\n  font-style:\
    \ normal;\n  font-display: swap;\n}\n\n@font-face {\n  font-family: \"Inter\";\n  src: url(\"source-fonts/05-inter-700-normal.ttf\"\
    ) format(\"truetype\");\n  font-weight: 700;\n  font-style: normal;\n  font-display: swap;\n}\n\n:root { --stt-source-font-family:\
    \ 'Inter'; }\n\nhtml, body, button, input, textarea, select { font-family: var(--stt-source-font-family) !important; }\n\
    \nh1, h2, h3, h4, h5, h6, .display, .heading, .heading-lg, .heading-md, .heading-sm, [class*=\"display\"], [class*=\"\
    heading\"] { font-family: var(--stt-source-font-family) !important; }"
---
...
## Overview
- Name: `editorial_evergreen_system`
- A calm, premium editorial aesthetic pairing high-contrast serif headings with a neutral humanist sans on deep-green inverse surfaces, mint and white canvases, and warm human-centered photography.
- Confidence: `high`
- Schema: `design_system_yaml.v1`

## Surface System
- `page_canvas`: value: #FFFFFF; role: page_canvas; confidence: high; text: default: #15372C, muted: #15372C, accent: #4CB398
- `mint_canvas`: value: #CDEBDD; role: section_run; confidence: medium; text: default: #15372C, muted: #15372C, accent: #15372C
- `inverse_strip`: value: #15372C; role: inverse_run; confidence: high; text: default: #F3F1EA, muted: #FFFFFF, accent: #FFFFFF
- `inverse_action_run`: value: #163328; role: inverse_run; confidence: high; text: default: #F3F1EA, muted: #5F9C84, accent: #FFFFFF
- `inverse_footer`: value: #14352B; role: inverse_run; confidence: high; text: default: #F3F1EA, muted: #5F9C84, accent: #F3F1EA
- `card_surface`: value: #FFFFFF; role: card; confidence: medium; text: default: #15372C, muted: #15372C, accent: #4CB398

## Tokens

### Color
- `color.surface`: primary: #FFFFFF, secondary: #CDEBDD, inverse: #15372C, inverseRun: #163328, inverseRunShift: #1D4034
- `color.surface.primary`: #FFFFFF
- `color.surface.secondary`: #CDEBDD
- `color.surface.inverse`: #15372C
- `color.surface.inverseRun`: #163328
- `color.surface.inverseRunShift`: #1D4034
- `color.surface.inverseFooter`: #14352B
- `color.text`: primary: #15372C, muted: #15372C, onInverse: #F3F1EA, onInverseMuted: #5F9C84, onAccent: #FFFFFF
- `color.text.primary`: #15372C
- `color.text.muted`: #15372C
- `color.text.onInverse`: #F3F1EA
- `color.text.onInverseMuted`: #5F9C84
- `color.text.onAccent`: #FFFFFF
- `color.border`: onLight: #D8E0DB, onInverse: #FFFFFF, accent: #4CB398
- `color.border.onLight`: #D8E0DB
- `color.border.onInverse`: #FFFFFF
- `color.border.accent`: #4CB398
- `color.accent`: primary: #4CB398, action: #2F6FE0, deep: #15372C
- `color.accent.primary`: #4CB398
- `color.accent.action`: #2F6FE0
- `color.accent.deep`: #15372C
- `color.graphic`: decorativeMotif: #1D4034, iconOnInverse: #5F9C84
- `color.graphic.decorativeMotif`: #1D4034
- `color.graphic.iconOnInverse`: #5F9C84

### Typography
- `typography.note`: Serif used only for page/section/content headings; Inter sans for body, controls, links, labels.

### Spacing
- `spacing.scale`: 9 item(s)
- `spacing.sectionPaddingTall`: 96px
- `spacing.sectionPaddingMedium`: 64px
- `spacing.gridGutter`: 32px
- `spacing.textStackGap`: 16px
- `spacing.cardInnerGap`: 16px
...
### Shadow
- `shadow.cardSoft`: 0 8px 24px transparent
- `shadow.widgetElevated`: 0 12px 32px transparent

### Divider
- `divider.hairlineOnLight`: 1px solid #D8E0DB

### Motion
- `motion.note`: Restrained, system-native transitions; subtle hover color/opacity shifts on links and buttons; optional gentle reveal on photography and cards.

## Typography
- `display_heading`: role: page_heading / oversized marquee heading; fontFamily: Editorial serif (source family unconfirmed; pair a high-contrast transitional/modern serif); fontSize: clamp(40px, 6vw, 72px); fontWeight: 400; lineHeight: 1.05; confidence: medium
- `h1`: role: page_heading; fontFamily: Editorial serif (as display_heading); fontSize: clamp(36px, 5vw, 60px); fontWeight: 400; lineHeight: 1.1; confidence: medium
- `h2`: role: section_heading; fontFamily: Editorial serif (as display_heading); fontSize: clamp(28px, 3.5vw, 44px); fontWeight: 400; lineHeight: 1.15; confidence: high
- `h3`: role: content_heading / card_title; fontFamily: Editorial serif (as display_heading); fontSize: 22px; fontWeight: 500; lineHeight: 1.25; confidence: high
- `body`: role: body / paragraph; fontFamily: 'Inter'; fontSize: 15px; fontWeight: 400; lineHeight: 1.6; confidence: high
- `body_muted`: role: supporting / metadata body; fontFamily: 'Inter'; fontSize: 14px; fontWeight: 400; lineHeight: 1.6; confidence: medium
- `label_eyebrow`: role: label_metadata / column eyebrow; fontFamily: 'Inter'; fontSize: 14px; fontWeight: 500; lineHeight: 1.4; confidence: medium
- `control_text`: role: control_text / link; fontFamily: 'Inter'; fontSize: 15px; fontWeight: 500; lineHeight: 1.4; confidence: high

## Components
- `inline_text_link`: kind: link; confidence: high; variants: on_inverse, on_light
- `primary_button`: kind: button; confidence: high; variants: solid_accent_on_light, solid_deep_on_mint, solid_action_on_inverse
- `outline_button`: kind: button; confidence: high; variants: on_light, on_inverse
- `icon_button`: kind: icon_button; confidence: medium; variants: utility_on_light, dismiss_on_inverse, social_chip_on_footer
- `feature_card`: kind: card; confidence: high; variants: chromeless_on_light
- `media_overlay_chip`: kind: compact_label; confidence: medium; variants: white_on_media
- `text_input`: kind: input; confidence: medium; variants: on_inverse
- `divider_vertical`: kind: other; confidence: medium; variants: on_light
- `floating_widget`: kind: panel; confidence: medium; variants: elevated
