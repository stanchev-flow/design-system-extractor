#!/usr/bin/env python3
"""Full strict gate battery for the ai-product-launch lane (true exit codes).

Gates (per the deliverable):
  a. onbrand_check.py --composition   (HARD invariants + anatomy-presence AS-81)
  b. slop_audit.mjs                    (@1440 + @1180 internally; AS-68..81)
  ...
  j. anatomy-presence                  (onbrand_check.check_anatomy_presence, explicit row)

Usage (repo root):
  env -u PLAYWRIGHT_BROWSERS_PATH ./venv/bin/python \
      runs/hubspot-v3/brand/compose/ai-product-launch/run_battery.py
"""
...
def anatomy_presence() -> int:
    sys.path.insert(0, str(BP))
    import onbrand_check as ob
    rows = ob.check_anatomy_presence(LANE)  # (rule, label, passed_bool, detail)
    out = ["anatomy-presence gate (onbrand_check.check_anatomy_presence, AS-81)"]
    fails = [r for r in rows if not r[2]]
    for r in rows:
        out.append(f"{'PASS' if r[2] else 'FAIL'} {r[0]} — {r[1]}: {r[3]}")
    if not rows:
        out.append("PASS — no section declares a tab/stat device requiring anatomy "
                   "(or fail-open on an unmappable page)")
    (BAT / "anatomy-presence.log").write_text("\n".join(out) + "\n")
