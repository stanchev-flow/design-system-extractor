# Fact-consumption audit (AS-83) — target: replica

**PASS** — 19 consumed / 0 unconsumed / 0 excluded / 5 delegated.

| family | status | origin | consumer | evidence | provenance |
|---|---|---|---|---|---|
| `responsive.hero` | consumed | extracted | component_render.hero_responsive_css | CSS marker '(fact-gated: layouts[].responsive)' | hero bound height mechanic is viewport-relative (viewport-minus-nav); nav offset from measured --global-nav-header-height |
| `responsive.hero.primaryButton` | consumed | extracted | component_render.hero_primary_button_css | CSS marker '(fact-gated: layouts[].responsive.primaryButton)' | primary action measured @1440 (display/font-size/line-height/border/padding) — the button box the composer left un-grounded |
...
| `layout.specialTreatment.sticky-column` | consumed | extracted | compose_section.stamp_pattern_devices (sticky-column device) | 'position: sticky' present in the emitted markup | layout-library specialTreatment kind=sticky-column (origin=extracted) |
| `layout.specialTreatment.edge-cut` | consumed | extracted | compose_section.stamp_pattern_devices (edge-cut device) | 'cs-edgecut' present in the emitted markup | layout-library specialTreatment kind=edge-cut (origin=extracted) |
| `layout.surfaceIntent` | consumed | extracted | compose_section.resolve_surface → section paint | data-surface="surface/inverse-teal" on the rendered section | layout surfaceIntent (origin=extracted) |
| `layout.surfaceIntent` | consumed | extracted | compose_section.resolve_surface → section paint | data-surface="surface/primary" on the rendered section | layout surfaceIntent (origin=extracted) |
| `layout.surfaceIntent` | consumed | extracted | compose_section.resolve_surface → section paint | data-surface="surface/primary" on the rendered section | layout surfaceIntent (origin=extracted) |
| `layout.surfaceIntent` | consumed | extracted | compose_section.resolve_surface → section paint | data-surface="surface/primary" on the rendered section | layout surfaceIntent (origin=extracted) |
| `layout.surfaceIntent` | consumed | extracted | compose_section.resolve_surface → section paint | data-surface="surface/accent-soft" on the rendered section | layout surfaceIntent (origin=extracted) |
| `layout.surfaceIntent` | consumed | extracted | compose_section.resolve_surface → section paint | data-surface="surface/primary" on the rendered section | layout surfaceIntent (origin=extracted) |
| `layout.surfaceIntent` | consumed | extracted | compose_section.resolve_surface → section paint | data-surface="surface/primary" on the rendered section | layout surfaceIntent (origin=extracted) |
| `layout.surfaceIntent` | consumed | extracted | compose_section.resolve_surface → section paint | data-surface="surface/primary" on the rendered section | layout surfaceIntent (origin=extracted) |
| `layout.surfaceIntent` | consumed | extracted | compose_section.resolve_surface → section paint | data-surface="surface/primary" on the rendered section | layout surfaceIntent (origin=extracted) |
