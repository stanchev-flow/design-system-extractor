...
| `git status` dirty right after install | Expected. `src/screenshot_to_template.egg-info/` is tracked but regenerated by `pip install -e .`. Harmless; discard with `git checkout -- src/screenshot_to_template.egg-info`. |
