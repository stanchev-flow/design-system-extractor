...
from run_pipeline import LANE_OUTCOME_STEP_STATUS
...
def generating_plans(**overrides):
    """A plan set with a lane actually switched on, for ledger-level tests.

    The shipped defaults now enable neither lane, so a ledger test that wants a
    lane to be able to produce has to say which one.
    """
    return baseline_plans(**{"vanilla_config_value": True, **overrides})
...
class LanePlanTests(unittest.TestCase):
    ...
    def test_the_vanilla_key_switches_the_vanilla_lane_off_on_its_own(self) -> None:
        # The regression: vanilla-site-generation-enabled: false used to suppress
        # this lane only while framework generation was on, so the CLI baseline
        # (framework off) ran it regardless of its own key. The key now decides.
        vanilla = lane(baseline_plans(), LANE_VANILLA)

        self.assertFalse(vanilla.enabled)
        self.assertEqual(vanilla.artifacts(), ())
        self.assertIn("vanilla-site-generation-enabled", vanilla.reason)
        self.assertIn("--vanilla-sites", vanilla.reason)

    def test_the_vanilla_key_alone_turns_the_lane_on_with_framework_off(self) -> None:
        vanilla = lane(baseline_plans(vanilla_config_value=True), LANE_VANILLA)

        self.assertTrue(vanilla.enabled)
        self.assertEqual(vanilla.artifacts(), ("site-claude.html",))
        self.assertIn("vanilla-site-generation-enabled is true", vanilla.reason)
    ...
    def test_a_lane_is_enabled_exactly_when_its_own_key_or_flag_says_so(self) -> None:
        # Each key now governs its own lane, so "nothing enabled" is reachable —
        # it is exactly the case where neither key nor either flag asks for a
        # lane. That case is what run_pipeline's up-front refusal exists for, and
        # this table is what pins it as reachable rather than theoretical.
                ...
                for vanilla_key in (False, True):
                    for vanilla_flag in (False, True):
                        ...
                        with self.subTest(
                            ...
                            self.assertEqual(
                                lane(plans, LANE_FRAMEWORK).enabled,
                                framework_key or framework_flag,
                            )
                            self.assertEqual(
                                lane(plans, LANE_VANILLA).enabled,
                                vanilla_key or vanilla_flag,
                            )
                            self.assertEqual(
                                any(p.enabled for p in plans),
                                any(
                                    (
                                        framework_key,
                                        framework_flag,
    ...
    def test_the_shipped_cli_default_enables_no_lane_at_all(self) -> None:
        # Ties the disclosure to the real config rather than a fixture: if either
        ...
        self.assertFalse(config.vanilla_site_generation_enabled)
        self.assertFalse(lane(plans, LANE_FRAMEWORK).enabled)
        self.assertFalse(lane(plans, LANE_VANILLA).enabled)
        # So a flagless CLI run is refused up front instead of quietly emitting
        # the vanilla HTML its own config had switched off.
        self.assertFalse(any(p.enabled for p in plans))


class LaneLedgerTests(unittest.TestCase):
    def test_produced_lane_reports_the_files_it_wrote(self) -> None:
        ledger = LaneLedger(generating_plans())
        ledger.record(
            LANE_VANILLA,
            "claude",
            "hatch",
    ...
    def test_gate_refusal_is_not_reported_as_a_failure(self) -> None:
        ledger = LaneLedger(generating_plans(framework_flag=True))
        ledger.record(
            LANE_FRAMEWORK,
            "claude",
            "hubspot",
    ...
    def test_one_target_s_outcome_can_be_read_back(self) -> None:
        # run_pipeline derives run-steps.json from this, so a generator that
        # failed is not also marked completed on a third surface.
        ledger = LaneLedger(generating_plans())
        ledger.record(LANE_VANILLA, "claude", "hatch", OUTCOME_FAILED, reason="boom")

        self.assertEqual(
            ledger.outcome_for(LANE_VANILLA, "claude", "hatch"), OUTCOME_FAILED
        )
        self.assertIsNone(ledger.outcome_for(LANE_VANILLA, "gpt55", "hatch"))
        self.assertIsNone(ledger.outcome_for(LANE_VANILLA, "claude", "other-item"))

    def test_every_recordable_outcome_has_a_step_status(self) -> None:
        # A missing entry would silently read as "failed" for a lane that was
        # only skipped, so the mapping has to stay total.
        for outcome in (
            OUTCOME_PRODUCED,
            OUTCOME_FAILED,
            OUTCOME_SKIPPED_GATE,
            ...
            with self.subTest(outcome=outcome):
                self.assertIn(outcome, LANE_OUTCOME_STEP_STATUS)
        self.assertEqual(LANE_OUTCOME_STEP_STATUS[OUTCOME_PRODUCED], "completed")
        self.assertEqual(LANE_OUTCOME_STEP_STATUS[OUTCOME_FAILED], "failed")
        self.assertEqual(LANE_OUTCOME_STEP_STATUS[OUTCOME_SKIPPED_GATE], "skipped")
...
class LaneSummaryTests(unittest.TestCase):
    ...
    def test_plan_lines_state_the_lanes_before_any_model_work(self) -> None:
        text = "\n".join(LaneLedger(generating_plans()).plan_lines())
...
class NoOutputFailureTests(unittest.TestCase):
    ...
    def test_extract_only_modes_are_never_failed_for_producing_no_site(self) -> None:
        ...
        ledger = LaneLedger(
            generating_plans(design_only=True),
